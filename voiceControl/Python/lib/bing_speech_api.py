...
class BingSpeechAPI:
    ...
    def authenticate(self):
        if self.expire_time is None or monotonic() > self.expire_time:  # first credential request, or the access token from the previous one expired
            ...
            credential_url = "https://francecentral.api.cognitive.microsoft.com/sts/v1.0/issuetoken"
            ...
            if response.status_code != 200:
                raise RequestError("authenticate http request error with status code {}".format(response.status_code))
    ...
    def recognize(self, audio_data, language="fr-FR", show_all=False):
        self.authenticate()
