...
/**
 * The Url class represents a single URL and provides methods for
 * manipulating URLs.
 *
 * @author Jan Schneider <jan@horde.org>
 * @author Michael Slusarz <slusarz@horde.org>
...
class Url {
	 ...
	 *        	The basic URL, with or without query
	 *        	parameters.
	 * @param boolean $raw
	 *        	Whether to output the URL in the raw URL
	 ...
	 * Useful for chaining.
	 *
	 * @return Url A clone of this object.
	 */
	...
	/**
	 * Adds or update one or more query parameters.
	 *
	 * @param mixed $parameters
	 *        	Either the name value or an array of
	 *        	name/value pairs.
	 * @param string $value
	 *        	If specified, the value part ($parameters is
	 ...
	 *
	 * @param mixed $remove
	 *        	Either a single parameter to remove or an array
	 ...
	 *
	 * @param boolean $raw
	 *        	Whether to output the URL in the raw URL format or
	 *        	HTML-encoded.
	 *        	
	 * @return Url This object, to allow chaining.
	 */
	 ...
	 *        	Force using $scheme, even if it already
	 *        	exists?
	 *        	
	 * @return Url This object, to allow chaining.
	 */
	 ...
	 *
	 * @param boolean $raw
	 *        	Whether to output the URL in the raw URL format
	 *        	or HTML-encoded.
	 * @param boolean $full
	 *        	Output the full URL?
	 *        	
	...
	public function toString( $raw = false, $full = true) {
		...
		$url = $full ? $this->url : parse_url( $this->url, PHP_URL_PATH);
		...
		if ( !isset( $this->_cache)) {
			...
			foreach ( $this->parameters as $p => $v) {
				if ( is_array( $v)) {
					...
					$params[] = rawurlencode( $p) . ( strlen( $v) ? ( '=' . rawurlencode( $v)) : '');
				}
			}
	 ...
	 *        	A hash with any additional attributes to be
	...
	public function link( array $attributes = array()) {
		...
		foreach ( $attributes as $name => $value) {
			...
			if ( substr( $name, -4) == '.raw') {
				$link .= ' ' . htmlspecialchars( substr( $name, 0, -4)) . '="' . $value . '"';
			} else {
				$link .= ' ' . htmlspecialchars( $name) . '="' . htmlspecialchars( $value) . '"';
			}
		}
		return $link . '>';
	}
	...
	public function unique() {
		return $this->add( 'uid', uniqid( mt_rand()));
	}
